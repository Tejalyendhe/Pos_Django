...
//  Generates dynamic data/table for report
var table;
$(document).ready(function () {
  ...
  function loadTableData() {
    ...
    $.ajax({
      type: 'POST',
      url: '/pos/load_table_data/',  // Replace with the correct URL
      dataType: 'json',
      headers: {
      ...
      success: function (data) {
        ...
        table = $('#inw_table').DataTable({
          ...
          columns: [
            ...
            { data: 'Inward_no', title: 'Inw no.' },
            { data: 'Supplier_names', title: 'Supplier Name' },
            { data: 'Bill_numbers', title: 'Bill No' },
            ...
            },
            {
              data: null, title: '', render: function (data, type, row) {
                if (row.Total_greturn_qty > 0 || row.Total_inward_qty > 0) {
                  return '<button type="button" class="btn btn-warning mark-as-done-button" onclick="event.stopPropagation(); markAsDone(this.closest(\'tr\'))"><i class="bi bi-check-square"></i></button>';
          ...
          createdRow: function (row, data, dataIndex) {
            ...
            $(row).attr('data-href', '/pos/view_product/' + data.Inward_no + '/');
          }
        });
...
function markAsDone(row){
    ...
    formData.append('inw_no',row.querySelector('td:nth-child(2)').textContent)
    $.ajax({
            type: 'POST',
            url:  '/pos/mark_as_done/',
            dataType: 'json',
            data: formData,
            cache: false,
            processData: false,
  ...
  document.getElementById("download-excel-button").addEventListener("click", function () {
    ...
    var filteredDF = new dfd.DataFrame(filteredData, {
        columns: ['Sr.no', 'Inw no.', 'Supplier Name', 'Bill No', 'Distpatch No','Invoice No' ,'Created Date', 'Total',''],
    });
